  ...
  computed: {
    ...
    targetGameResults(): GameResult[] {
      return this.gameResults.filter((gameResult) => {
        ...
        return gameResultPlayers.length === this.playerCount &&
  ...
  methods: {
    ...
    point(playerId: number): number {
      return this.targetGameResults.reduce((total, gameResult) => {
        const gameResultPlayer = gameResult.gameResultPlayers.find((gameResultPlayer) => gameResultPlayer.player_id === playerId);
